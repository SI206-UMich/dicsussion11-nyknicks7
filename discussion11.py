...
def add_fluffle(cur, conn):
    ...
    cur.execute("INSERT INTO Species (id,name, species_id, age, cuteness, aggressiveness) VALUES (?,?, ?, ?, ?, ?)",
...
def add_pets_from_json(filename, cur, conn):
    ...
    for i in json_data:
        pet_id = count
        name = i["name"]
        curr.execute('SELECT id from Species WHERE title = ?',(i['species'],))
        species = int(curr.fetchone{}{0})
        age = int(i{"age"})
